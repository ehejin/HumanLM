...
4. Compute penalty P for extra or conflicting content:
   - Examine additional content in the generated {hierarchy_name} beyond those key points:
     ...
     - Does it describe or greatly overlap with a different hierarchy dimension than {hierarchy_name}? Add 0.2-0.4 penalty if so. For example, if the hierarchy is communication, but it contains content about the stance of the user, penalize it.
   - Set a penalty P ∈ [0, 1]:
...
7. Human-likeness penalty: After computing base_score = max(0, min(1, C - P)), if the generated response does not read humanlike for a user message in this context (e.g., assistant-y tone, robotic/formal, generic template, very unnatural verbosity), then set score = 0.5 * base_score. Mention in the "thought" whether you applied the 0.5 multiplier.
