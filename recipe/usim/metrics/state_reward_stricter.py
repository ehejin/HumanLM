...
IMPORTANT: You are ONLY evaluating the "{hierarchy_name}" dimension. Other dimensions are handled separately. If the generated content describes a different dimension than "{hierarchy_name}", it is OFF-TASK and should receive a penalty.
...
2. Score how well the generated {hierarchy_name} matches each key point:
   - For each key point i, compare it with the generated {hierarchy_name} and assign a match value m_i in range [0, 1]:
     ...
     - If the generated content HEAVILY and CLEARLY describes a DIFFERENT hierarchy (e.g., talks only about writing style when evaluating "belief"), assign score = 0.0 and skip the rest of the steps.
...
4. Compute penalty P for extra or conflicting content:
   - Examine additional content in the generated {hierarchy_name} beyond those key points:
     ...
     - **Does it describe a different hierarchy dimension than {hierarchy_name}?** (Add 0.3-0.5 penalty if so)
   - Set a penalty P ∈ [0, 1]:
