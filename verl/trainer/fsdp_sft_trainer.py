...
class FSDPSFTTrainer:
    ...
    def dump_val_samples(self, batch: TensorDict, global_step: int):
        """Dump a few val examples for human inspection.
        """
        rank = self.device_mesh.get_rank()
        max_new_tokens = int(getattr(self.config.trainer, "dump_val_samples_max_new_tokens", 128) or 128)
        ...
        n = min(3, bs)  # Generate 3 samples max
        ...
        with torch.no_grad():
            for i in range(n):
                ids = input_ids[i]
                am = attention_mask[i]
                lm = loss_mask[i] if loss_mask is not None else None

                valid_len = int(am.sum().item()) if am is not None else int(ids.shape[0])

                # Find first token position where loss is applied (response region).
                resp_start = valid_len
                if lm is not None:
                    idxs = (lm[:valid_len] > 0).nonzero(as_tuple=False)
                    if idxs.numel() > 0:
                        resp_start = int(idxs[0].item())

                prompt_ids = ids[:resp_start]
                target_ids = ids[resp_start:valid_len]

                gen = self.fsdp_model.generate(
                    input_ids=prompt_ids.unsqueeze(0).to(self.device_name),
                    attention_mask=am[:resp_start].unsqueeze(0).to(self.device_name) if am is not None else None,
                    do_sample=False,
                    max_new_tokens=max_new_tokens,
                    pad_token_id=self.tokenizer.pad_token_id,
                )
                gen_ids = gen[0][len(prompt_ids):]

                if rank == 0:
                    prompt_text = self.tokenizer.decode(prompt_ids.tolist(), skip_special_tokens=False)
                    target_text = self.tokenizer.decode(target_ids.tolist(), skip_special_tokens=False)
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        if not dumped:
                            self.dump_val_samples(val_data, global_step=global_step)
                            dumped = True
...
def run_sft(config):
    ...
    # [USIM] pass in the data kwarg w/ custom chat template
