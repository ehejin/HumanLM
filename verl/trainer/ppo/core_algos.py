...
@register_adv_est(AdvantageEstimator.GRPO_PRM)
def compute_grpo_process_advantage(
    token_level_rewards: torch.Tensor,   # (B, T)
    response_mask: torch.Tensor,         # (B, T) in {0,1}
    index: np.ndarray,                   # (B,) prompt-group ids
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    GRPO-style advantages for dense rewards with multiple parts per response.

    Assumptions:
      - In each sequence there are exactly P (>0) tokens with non-zero reward
        (within the response_mask). Those P token positions define the P parts
        in left-to-right order; tokens before the first non-zero belong to part 0,
        tokens between k-th and (k+1)-th non-zero belong to part k, and tokens
        after the last non-zero belong to part P-1.
      - All sequences in the batch share the same P.

    Steps:
      1) Infer part boundaries from non-zero reward token positions.
      2) Per (sample, part) scalar reward r[b,p] = SUM of rewards over tokens of that part.
      3) For each prompt-group g and part p, center (and optionally std-normalize):
           a[b,p] = (r[b,p] - mean_g,p) / (std_g,p + eps)   (classic GRPO)
         or a[b,p] = r[b,p] - mean_g,p                      (no-std GRPO)
      4) Broadcast per-part scalar unchanged to all tokens in that part.

    Returns:
      advantages: (B, T)
      returns:    (B, T)   # same as advantages, GRPO-style
    """
    assert token_level_rewards.dim() == 2 and response_mask.dim() == 2
    B, T = token_level_rewards.shape
    device = token_level_rewards.device

    with torch.no_grad():
        # Identify non-zero reward tokens within the valid response region
        nz_mask = (token_level_rewards != 0) & (response_mask != 0)
        nz_counts = nz_mask.sum(dim=1)  # (B,)
        # Infer P from the first sample; assert all match
        P = int(nz_counts[0].item())
        assert P > 0, "Each sequence must contain at least one non-zero reward token."
        if not torch.all(nz_counts == P):
            raise ValueError(f"All sequences must have exactly {P} non-zero reward tokens; got {nz_counts.tolist()}")

        # Build per-sample part_ids via cumulative sum over nonzero markers
        # For each row: flag[t]=1 at non-zero positions, else 0; part_id=cumsum(flag)-1, clamped to [0,P-1]
        part_ids = torch.zeros((B, T), dtype=torch.long, device=device)
        part_anchor_idx = torch.empty((B, P), dtype=torch.long, device=device)

        for b in range(B):
            nz_idx = torch.nonzero(nz_mask[b], as_tuple=False).squeeze(-1)  # (P,)
            part_anchor_idx[b] = nz_idx
            flag = torch.zeros(T, dtype=torch.long, device=device)
            flag[nz_idx] = 1
            pid = torch.cumsum(flag, dim=0) - 1
            pid = pid.clamp_(min=0, max=P - 1)
            part_ids[b] = pid

        # Per (sample, part) scalar reward: SUM over tokens in that part (no length normalization)
        rewards_masked = token_level_rewards * response_mask
        r_bp = torch.zeros((B, P), dtype=rewards_masked.dtype, device=device)
        for p in range(P):
            r_bp[:, p] = (rewards_masked * (part_ids == p)).sum(dim=1)

        # Center (and optionally std-normalize) per group × part
        a_bp = torch.zeros_like(r_bp)
        inv = torch.from_numpy(np.unique(index, return_inverse=True)[1]).to(device)  # compact group ids
        for g in torch.unique(inv):
            sel = (inv == g)
            r_g = r_bp[sel]  # (Ng, P)
            if r_g.numel() == 0:
                continue
            ...
            if norm_adv_by_std_in_grpo:
                ...
                a_bp[sel] = (r_g - mu) / (std + epsilon)
            else:
                a_bp[sel] = r_g - mu

        # Broadcast unchanged per-part scalar to all tokens in that part
        advantages = torch.zeros_like(token_level_rewards)
        for p in range(P):
            mask_p = (part_ids == p).float() * response_mask
            advantages += a_bp[:, p].unsqueeze(1) * mask_p
