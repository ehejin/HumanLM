...
def compute_reward_with_reasoning(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[torch.Tensor, dict[str, Any], list[dict]]:
    """
    Compute reward for a batch of data.
    Args:
        data: DataProto object containing the input data.
    ...
    try:
        ...
        reward_tensor = reward_result["reward_tensor"]
        reward_extra_infos_dict = reward_result.get("reward_extra_info", {})
        reward_dicts = reward_result.get("reward_dicts", [])
    except Exception as e:
        print(f"Error in reward_fn: {e}")
        reward_tensor = reward_fn(data)
        reward_extra_infos_dict = {}
    ...
    return reward_tensor, reward_extra_infos_dict, reward_dicts
