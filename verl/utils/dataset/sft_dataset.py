            ...
            def gen_starts_with_at(gen):
                if isinstance(gen, dict):
                    ...
                    try:
                        ...
                        raise ValueError("Error in loading json str")
                else:
                    raise ValueError("gen is wrong type (not dict or str)")
                return isinstance(content, str) and content.startswith("@")

            # keep rows that DO NOT start with @
            self.dataframe = self.dataframe[~self.dataframe["generation"].apply(gen_starts_with_at)].reset_index(drop=True)

        after = len(self.dataframe)
        print("DATASET, ", self.dataset)
        print(self.parquet_files)
        print("BEFORE filtering for @, AFTER: ", before, after)
    ...
    def __getitem__(self, item):
        ...
        if self.speak_as:
            ...
            prompt_chat_str = tokenizer.apply_chat_template(
                prompt, add_generation_prompt=True, tokenize=False, speak_as=name, **self.apply_chat_template_kwargs
            )
        else:
            prompt_chat_str = tokenizer.apply_chat_template(
