...
class Tracking:
    ...
    def finish(self):
        """Explicitly finalize all initialized loggers."""
    ...
    def __del__(self):
        # Fallback if finish is not called explicitly.
        try:
            self.finish()
        except Exception:
            pass
...
@dataclasses.dataclass
class BestHierarchyLogger:
    """Minimal logger for best parent hierarchies (similar to ValidationGenerationsLogger)."""
    project_name: str = None
    experiment_name: str = None

    def log(self, loggers, best_hierarchy_map, step, max_entries=10):
        """Log top best hierarchies to wandb."""
        if not best_hierarchy_map or "wandb" not in loggers:
            return
            
        import wandb

        # Flatten the map: each key can have up to 20 entries (list of dicts)
        # We'll take the best entry from each key's list for the top-level table
        flattened_items = []
        for key, entries_list in best_hierarchy_map.items():
            if entries_list and len(entries_list) > 0:
                # Take the best (first) entry from each key's top 20 list
                best_entry = entries_list[0]
                flattened_items.append((key, best_entry))
        
        # Sort by reward and take top max_entries keys
        sorted_items = sorted(
            flattened_items,
            key=lambda x: x[1].get("best_reward", 0.0),
            reverse=True
        )[:max_entries]

        # Get hierarchy name from the first entry (all should have same hierarchy level)
        hierarchy_name = sorted_items[0][1].get("hierarchy_name", "hierarchy") if sorted_items else "hierarchy"
        columns = ["step", "key", f"best_{hierarchy_name}", "best_reward"]
        ...
        # Add top entries for this step
        for key, info in sorted_items:
            new_table.add_data(step, key, info.get("best", ""), info.get("best_reward", 0.0))
        
        wandb.log({f"hierarchy/best_{hierarchy_name}": new_table}, step=step)
