...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        print("detokenize", config.get("detokenize", False))
        ...
        # Handle no_repeat_ngram_size via logits processor (not a native vLLM SamplingParams parameter)
        ...
        if no_repeat_ngram_size > 0:
            ...
            logger.info(f"Enabled no_repeat_ngram_size={no_repeat_ngram_size} via logits processor")
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # [USIM]
        stop = prompts.meta_info.get("stop", None)
        if stop: kwargs.update({"stop": stop})

        max_tokens = prompts.meta_info.get("max_tokens", None)
        if max_tokens: kwargs.update({"max_tokens": max_tokens})
